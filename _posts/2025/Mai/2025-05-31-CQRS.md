...
## 5. Réception du message et traitement avec MediatR
...
### 5.1 IRequest, IActionResult, INotification et IRequestHandler
...
## 6. Exemple de code avec Publish, INotification et INotificationHandler
...
sequenceDiagram
    ...
    UI->>API: POST /annuler
    API->>ASB: SendMessage(NewTransitionHistoriqueEvent)
    ASB->>BACKEND: Reçoit Event
    BACKEND->>MediatR: Publish(Event)
