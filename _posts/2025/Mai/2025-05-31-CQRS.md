...
```csharp
[HttpDelete("{id}")]
public async Task<IActionResult> SupprimerCommande(Guid id, Client client)
{
    ...
    if (result.IsSuccess)
    {
        ...
        await mediator.Publish(new CommandeAnnuleeNotification(id, "Suppression", client.Email));
    }
