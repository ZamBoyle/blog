...
sequenceDiagram
    participant UI
    participant API (MediatR)
    participant MediatR
    participant Handler

    UI->>API (MediatR): POST /annuler
    API (MediatR)->>MediatR: Send(AnnulerCommandeRequest)
    MediatR->>Handler: Handle(AnnulerCommandeRequest)
    Handler-->>MediatR: Résultat (ex: Result)
    MediatR-->>API (MediatR): Résultat
    API (MediatR)-->>UI: Réponse HTTP (ex: 200 OK, 400 BadRequest...)
```
