...
public record CommandeAnnuleeNotification(Guid CommandeId, string MotifAnnulation, string EmailClient) : INotification;
public class CommandeAnnuleeHandler(
    IEmailService emailService,
    IDocumentService documentService) 
    : INotificationHandler<CommandeAnnuleeNotification>
{
    public Task Handle(CommandeAnnuleeNotification notification, CancellationToken cancellationToken)
    {
...
```csharp
[HttpDelete("{id}")]
public async Task<IActionResult> SupprimerCommande(Guid id)
{
    // 1. Supprimer via command
    var result = await mediator.Send(new SupprimerCommandeRequest(id));
    
    if (result.IsSuccess)
    {
        // 2. Publier la notification depuis le contrôleur
        await mediator.Publish(new CommandeAnnuleeNotification(id, "Suppression", email));
    }
